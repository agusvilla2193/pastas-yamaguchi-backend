...
export class MailService {
    ...
    constructor(private configService: ConfigService) {
        // Aquí usamos la API Key que configuraremos en Render
        this.resend = new Resend(this.configService.get<string>('MAIL_PASS'));
    }
    ...
    async sendConfirmationEmail(email: string, firstName: string, token: string) {
        ...
        return await this.resend.emails.send({
            from: `Yamaguchi Pastas <onboarding@resend.dev>`, // Resend requiere este remitente al principio
            to: email,
    ...
    async sendResetPasswordEmail(email: string, firstName: string, token: string) {
        ...
        return await this.resend.emails.send({
            from: `Yamaguchi Pastas <onboarding@resend.dev>`,
            to: email,
